...
当修改或新增 dotnet new 模板相关内容时，必须同时遵循以下规则：

- 模板显示名 `name` 必须以 `Pek` 开头，采用 `PekXxx` 风格
- 模板短名 `shortName` 必须以 `pek` 开头，采用全小写前缀风格，例如 `pekmvc`、`pekvuezero-sln`
- 模板示例项目名、解决方案名、服务名优先使用 `DemoPekXxx` 风格，避免继续引入不带 `Pek` 前缀的示例
- 模板 README、打包脚本、SmokeTest 命令、聚合模板说明必须同步更新，禁止只改 `template.json`
- 新增模板后必须保证聚合模板包一次安装即可使用，不能要求用户逐个安装多个模板包才能看到模板
- 对外只保留 `PekBundle.Template` 一种安装方式，单模板包只允许打包验证，不再提供直接安装入口
- 对外发布到 NuGet 时，也只允许发布 `PekBundle.Template`，单模板包只允许本地打包验证
- 若修改模板命名规范，必须同步更新聚合校验逻辑，确保后续 AI 和人工改动都会被自动检查出来
- 模板包 `PackageId` 和 `Title` 必须采用 `PekXxx.Template` 风格；聚合模板包也必须遵循同一规则
